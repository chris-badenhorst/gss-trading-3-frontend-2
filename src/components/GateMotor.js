...
const GateMotorItem = ({ id, name, description }) => {
...
const ateMotor = () => {
  const makeItem = (id, name, description) => (
    <GateMotorItem key={id} id={id} name={name} description={description} />
  );
  ...
  const items = [
    ...
    },
    {
      id: 3,
      name: "Lid",
      fix: "Inspect the lid for any damage. Replace if necessary.",
    },
    {
      id: 4,
      name: "Door Lock",
    ...
    },
    {
      id: 6,
      name: "Power Supply",
      fix: "Use a multimeter to test the voltage at the garage motor's power supply input. If there is no power, check the circuit breaker or power source. Repair or replace the power supply components if necessary.",
    },
    {
      id: 7,
      name: "Battery",
